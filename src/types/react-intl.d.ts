...
// AAA specific: We customize the typings of react-intl and add the generic IDS to MessageDescriptors
// This way, we can make sure only available ids are used during for the Formatted* Components during compile time
...
declare namespace ReactIntl {
    ...
    function injectIntl<IDS, T extends React.ComponentClass<InjectedIntlProps<IDS>> | React.StatelessComponent<InjectedIntlProps<IDS>>>(component: T): T;
    ...
    interface Messages<T> {
        [key: string]: FormattedMessage.MessageDescriptor<T>
    }

    function defineMessages<T extends Messages<any>>(messages: T): T;
    ...
    interface InjectedIntl<IDS> {
        ...
        formatPlural: (value: number, options?: FormattedPlural.PropsBase<IDS>) => string;
        formatMessage: (messageDescriptor: FormattedMessage.MessageDescriptor<IDS>, values?: Object) => string;
        formatHTMLMessage: (messageDescriptor: FormattedMessage.MessageDescriptor<IDS>, values?: Object) => string;
    }

    interface InjectedIntlProps<IDS> {
        intl?: InjectedIntl<IDS>
    }
    ...
    namespace FormattedMessage {
        export interface MessageDescriptor<IDS> {
            id: IDS;
            description?: string;
            defaultMessage?: string;
        }

        export interface Props<IDS> extends MessageDescriptor<IDS> {
            values?: Object;
            tagName?: string;
        }
    }
    class FormattedMessage<IDS> extends React.Component<FormattedMessage.Props<IDS>, any> { }


    class FormattedHTMLMessage<IDS> extends React.Component<FormattedMessage.Props<IDS>, any> { }
    ...
    namespace FormattedPlural {
        export interface PropsBase<IDS> {
            /*
            * one of "cardinal" (default) | "ordinal"
            */
            style?: string;
            other?: React.ReactElement<any> | IDS;
            zero?: React.ReactElement<any> | IDS;
            one?: React.ReactElement<any> | IDS;
            two?: React.ReactElement<any> | IDS;
            few?: React.ReactElement<any> | IDS;
            many?: React.ReactElement<any> | IDS;
        }

        export interface Props<T> extends PropsBase<T> {
            value: number;
        }
    }
    class FormattedPlural<IDS> extends React.Component<FormattedPlural.Props<IDS>, any> { }
